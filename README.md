...
Ask the captain to spin up a crew. Its startup instructions carry a fixed placement ruleset: for every creation it asks **Claude Code or Codex**, then **pane or tab**, for a pane **vertical, horizontal, or auto**, for any direction **which pane to split** (or auto), and finally **Manual select or Smart select** for the model. Manual passes the model name you give; Smart lets the captain pick by task type (mechanical edits get the cheapest model, normal features the mid model, design, debugging, or multi-file work the strongest). Each question is asked on its own, only after the previous answer, and never batched, inferred, or defaulted. The pane list covers the whole workspace, grouped by tab with the tab name, each pane's label or title, and a captain marker. A vertical split opens the crew to the right of the chosen pane; a horizontal split opens it below, even in another tab. Auto reads the current tab's pane geometry from Herdr and splits the pane whose halves stay largest and squarest on screen (cells count as twice as tall as wide), so a row of narrow columns gets a horizontal split and a stack of short rows gets a vertical one. Ties go to panes without crew, then to panes nearest the captain. Every candidate must leave both halves at least 60 columns by 15 rows; when no pane can, the crew opens in a new tab instead. The chosen pane, direction, and a one-line reason are printed and recorded in session memory. You can also run the command directly from a shell associated with the Captain session:
