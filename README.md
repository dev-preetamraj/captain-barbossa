...
Ask the captain to spin up a crew. Its startup instructions carry a fixed placement ruleset: for every creation it asks **Claude Code or Codex**, then **pane or tab**, for a pane **vertical or horizontal**, and for both directions **which pane to split**. Each question is asked on its own, only after the previous answer, and never batched, inferred, or defaulted. The pane list covers the whole workspace, grouped by tab with the tab name, each pane's label or title, and a captain marker. A vertical split opens the crew to the right of the chosen pane; a horizontal split opens it below, even in another tab. You can also run the command directly from a shell associated with the Captain session:
...
In an interactive shell, every choice uses a compact keyboard selector. Move with **↑/↓ or j/k**, press **Enter** to choose, or **Esc / Ctrl+C** to cancel. The highlighted row is only selected when you press Enter. An agent tool invocation without a terminal returns an instruction to ask you about any missing choice and creates nothing; for a pane placement that message lists every workspace pane by tab, ID, and label. After your answer, the captain supplies `--agent claude|codex`, `--placement pane|tab`, and for a pane `--direction vertical|horizontal` with `--split-pane <pane-id>`. Explicit flags count as your choices, so they do not trigger another question. A split pane may be anywhere in the workspace; a closed or mistyped pane ID, an empty pane list, or split flags combined with `--placement tab` report an error and create nothing.
